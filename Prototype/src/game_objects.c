...
void updateProgrammableWorker(ProgrammableWorker *programmableWorker, GameObjectData *gameObjectData, int ticks){
  ...
  /* Sensory Perception */
	if(programmableWorker->brain.foundNode != NULL && (getDistance2BetweenRects(programmableWorker->rect,programmableWorker->brain.foundNode->rect) > 500*500 || !programmableWorker->brain.foundNode->alive)){
	   programmableWorker->brain.foundNode = NULL;
	}
...
void updateWeather(GameObjectData *gameObjectData, AudioData *audioData, Weather *weather, int ticks){
	...
	if(weather->tickCount > TICKSPERWEATHER && !gameObjectData->pause_status){
		...
		case Cloud:
			...
			printf("BEFORE: at rain start in gaem obects.c weatherSoundActive = %d\n", audioData->weatherSoundActive);
			audioData->weatherSoundActive = 1;
			printf("AFTER: at rain start in gaem obects.c weatherSoundActive = %d\n", audioData->weatherSoundActive);			
			break;
		case Rain:
			weather->present_weather = (rand() % CHANCE_OF_CLOUD == 0) ? Cloud : Sun;
			fadeOutChannel(weatherChannel, audioData);
			printf("BEFORE: at rain stop in gaem obects.c weatherSoundActive = %d\n", audioData->weatherSoundActive);
			audioData->weatherSoundActive = 0;
			printf("AFTER: at rain stop in gaem obects.c weatherSoundActive = %d\n", audioData->weatherSoundActive);			
			break;
		case Snow:
	 	...
	 	if(gameObjectData->roamingSpider->displayInfo){
  	 	...
  	 	blitGameObject(gameObjectData->roamingSpider->rect,
                  	graphicsData,
                   	...
                   	((gameObjectData->roamingSpider->eating_bee) ? 0 : NO_ICECREAM_INDEX_OFFSET)],
